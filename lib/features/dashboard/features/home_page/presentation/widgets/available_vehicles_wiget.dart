    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(vehicles.length, (i) => _listBuilder(i, context))
        ],
      ),
    );
  }

  Widget _listBuilder(int index, BuildContext context) {
    final vehicle = vehicles[index];
    return GestureDetector(
      onTap: () async {
        await Dependencies.resolve<GlobalRouter>()?.pushNamed(
            NamedRoutes.VEHICLE_DETAILS_SCREEN,
            arguments: VehicleDetailsScreenArguments(vehicle, false));
        cubit.onCategoryChange(cubit.state.category);
      },
      child: Container(
        height: 140.relative,
        child: Column(
